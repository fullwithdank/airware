...
#pragma region GetHitboxesToScan
	...
	int BaimMode = Menu::Window.RageBotTab.Baim.GetIndex();
	...
	HitBoxesToScan.clear();//should fix some hitbox shit
		...
		if (huso < health)
		{
			...
			else if (Menu::Window.MiscTab.Zeusbot.GetState() && GameUtils::Zeus(pWeapon))
			{
				HitBoxesToScan.push_back((int)CSGOHitboxID::Chest);
				HitBoxesToScan.push_back((int)CSGOHitboxID::Stomach);
				HitBoxesToScan.push_back((int)CSGOHitboxID::Pelvis);
			...
			else if (Menu::Window.RageBotTab.Baim.GetIndex() == 0)
			{
				HitBoxesToScan.push_back((int)CSGOHitboxID::Head);
				HitBoxesToScan.push_back((int)CSGOHitboxID::UpperChest);
				HitBoxesToScan.push_back((int)CSGOHitboxID::Chest);
			...
			else if (Menu::Window.RageBotTab.Baim.GetIndex() == 1)
			{
				if (pEntity->GetVelocity().Length2D() < 0.f)
				{
					HitBoxesToScan.push_back((int)CSGOHitboxID::UpperChest);
					HitBoxesToScan.push_back((int)CSGOHitboxID::Chest);
					HitBoxesToScan.push_back((int)CSGOHitboxID::LowerChest);
			...
			else if (Menu::Window.RageBotTab.Baim.GetIndex() == 2)
			{
				HitBoxesToScan.push_back((int)CSGOHitboxID::Head);
				HitBoxesToScan.push_back((int)CSGOHitboxID::UpperChest);
				HitBoxesToScan.push_back((int)CSGOHitboxID::Chest);
			...
			else if (Menu::Window.RageBotTab.Baim.GetIndex() == 3)
			{
				if (pEntity->GetVelocity().Length2D() < 0.f)
			{
				HitBoxesToScan.push_back((int)CSGOHitboxID::UpperChest);
...
namespace AntiAims
{
	...
	void SidewaysLeft(CUserCmd *pCmd, bool &bSendPacket)
	{
		int random = rand() % 100;
		...
		if (random < 15)
		{
			float change = -5 + (rand() % (int)(80 + 1));
			pCmd->viewangles.y += change;
		}
		if (random == 69)
		{
			float change = +5 + (rand() % (int)(90 + 1));
			pCmd->viewangles.y += change;
		}
	}

	void SidewaysRight(CUserCmd *pCmd, bool &bSendPacket)
	{
		int random = rand() % 100;
		...
		if (random < 15)
		{
			float change = -10 + (rand() % (int)(140 + 1));
			pCmd->viewangles.y += change;
		}
		if (random == 69)
		{
			float change = -20 + (rand() % (int)(180 + 1));
			pCmd->viewangles.y += change;
		}
	}
...
void SidewaysLeft(CUserCmd* pCmd, IClientEntity* pLocal, bool& bSendPacket)
{
		bSendPacket = true;
		pCmd->viewangles.y += 90;
...
void SidewaysRight(CUserCmd* pCmd, IClientEntity* pLocal, bool& bSendPacket)
{
	bSendPacket = true;
	pCmd->viewangles.y -= 90;
...
void BackwardsForwards(CUserCmd* pCmd, IClientEntity* pLocal, bool& bSendPacket)
{
	bSendPacket = true;
	pCmd->viewangles.y -= 180;
...
void ManualAntiAim(CUserCmd *pCmd)
{
...
void ManualAntiAimFake(CUserCmd *pCmd)
{
	static bool side = false;
	static bool side2 = false;
	static bool back = false;
...
void DoRealAA(CUserCmd* pCmd, IClientEntity* pLocal, bool& bSendPacket)
{
	...
	case 0:
		break;
	case 1:
		AntiAims::SidewaysRight(pCmd, bSendPacket);
		break;
	case 2:
		AntiAims::SidewaysLeft(pCmd, bSendPacket);
		break;
	case 3:
...
void CRageBot::DoAntiAim(CUserCmd *pCmd, bool &bSendPacket)
{
	...
	if (!bSendPacket && Menu::Window.RageBotTab.LBYBreak.GetState()) {
		if (next_lby_update_func(pCmd, pCmd->viewangles.y + Menu::Window.RageBotTab.LBYBreakDelta.GetValue())) {
			pCmd->viewangles.y += Menu::Window.RageBotTab.LBYBreakDelta.GetValue();
		}
	}

	/*
	if (Menu::Window.RageBotTab.DormantAA.GetState())
	{
		bool dormant = true;
		for (int i = 1; i < Globals::maxClients; i++)
		{
			IClientEntity* ent = Entlist->GetClientEntity(i);
			if (!ent || ent->GetClientClass()->m_ClassID != (int)CSGOClassID::CCSPlayer || ent->GetTeamNum() == pLocal->GetTeamNum() || !ent->IsAlive()) continue;
